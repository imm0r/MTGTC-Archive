...
--
-- Die Deck-Tabelle gruppierte lange allein nach Kartentyp — eine Ordnung, die
...
-- er als Ramp. Hier legt jeder seine eigene Ordnung darüber.
--
-- JE DECK, nicht je Nutzer: dieselbe Karte kann in einem Deck Ramp sein und im
-- nächsten Fixing, und ein Aggro-Deck braucht andere Fächer als ein
-- Control-Deck. Wer eine Liste wiederverwenden will, übernimmt sie in der App
-- aus einem anderen Deck — kopiert, nicht geteilt, damit ein Umbenennen hier
-- nicht dort etwas verstellt.
...
create table if not exists public.price_history (
  ...
  -- MTGJSON-uuid dieser Karte, aus AllIdentifiers. Erlaubt dem Tageslauf,
  -- allein AllPricesToday (5 MB) zu laden, statt AllIdentifiers (215 MB)
  -- erneut zu streamen. NULL heißt: noch kein voller Lauf hat sie ermittelt.
  mtgjson_uuid text,
  updated_at timestamptz not null default now()
);

-- Nachtragbar für Bestandsinstallationen, die die Tabelle schon haben.
...
begin
  ...
  on conflict (scryfall_id) do update
     set prices       = public.merge_price_map(ph.prices, excluded.prices),
         -- coalesce: ein Lauf ohne uuid darf die vorhandene nicht auf NULL
         -- zuruecksetzen, sonst fiele der Tageslauf still auf die grosse Datei
         -- zurueck.
...
-- ------------------------------------------------- Lückenprüfung
-- MTGJSON liefert Preise ausschließlich als ~1 GB große Bulkdatei — einen
-- Abruf für eine einzelne Karte gibt es nicht. Eine neu erfasste Karte bekäme
-- ihre 90 Tage deshalb erst beim nächsten Nachtlauf.
--
-- Damit der Job STÜNDLICH laufen kann, ohne stündlich 1 GB zu ziehen, braucht
-- er eine billige Vorfrage: fehlt überhaupt jemandem der Verlauf? Ist die
-- Antwort leer, beendet sich der Lauf in Sekunden; nur wenn wirklich etwas
-- fehlt, wird die Bulkdatei angefasst — und dann gezielt für diese Karten.
--
...
-- ---------------- Wunschkarte einlösen: Deckplatz umhängen ---------------
-- Eine Wunschkarte ist ein ganz normaler Deck-Eintrag, dessen Karte man (noch)
-- nicht besitzt — sie belegt also bereits einen der 100 Plätze. Kauft man sie
-- später, ist das oft eine ANDERE Auflage als die, die den Wunsch angelegt hat.
-- Die besessene Auflage zusätzlich ins Deck zu legen wäre falsch (sie wäre
-- Karte 101 und im Commander eine zweite Kopie derselben Karte); richtig ist,
-- den vorhandenen Eintrag umzuhängen.
--
-- Warum in der Datenbank: Abziehen und Gutschreiben müssen zusammen passieren.
-- Zwei getrennte Aufrufe aus dem Browser könnten zwischendrin abbrechen und das
-- Deck um eine Karte kürzen — und in der falschen Reihenfolge liefe das
-- Gutschreiben in den 100-Karten-Trigger.
